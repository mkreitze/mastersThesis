...
#R is also arbitrary. 
#global inits
R=10000 #note this is R_o^2... because it tends to be easier. 100 is standard
startXVal= -100 # value we want to vary x from-10 standard
endXVal= 100 #value we want to vary x to 10 standard
#startYVal= -10 # only used if a custom range is specified
#endYVal= 10 # only used if a custom range is specified
sizeOfStepX = 2 #size of step for x; standard 0.5
sizeOfStepY = 2 #size of step for y; standard 0.5 want ~8000 for good image
accuracy=4 # num of decimals
CALength=5 #Length of the generated image (for min use 5)
CAWidth=4 #Width of the generated image (for min use 4)
useMinMap=1 #checks if youre using 'min' map
useImages=0 #checks if you want images
numOfGens=1 #number of generations
numOfStates=2 #number of states (good until 10)... I cant use more than 2
amountOffsetX=0.5
amountOffsetY=0.5
QUANTIFER="dB Update Int"
...
for a in range (5,45,5):
    ...
    if a==0:
        a=1
    QUANTIFER="dB Update Run "+str(a)
    numOfGens=a
    #generate all score matricies    
    print(str(a)+" Generating score matricies")
    CAMapInit=[]
    ...
    for x in range(startXVal,endXVal,sizeOfStepX):
        ...
        if(R-pow(x,2)<0): #finds possible y values (makes sure not to break math first)
            continue
        endYVal=int(math.sqrt(R-pow(x,2)))
        startYVal=int((-1)*endYVal)
        #incase were using a predetermined y val set mute above
        for y in range(startYVal,endYVal,sizeOfStepY): #binds as -sqrt(R_o^2-x^2) to sqrt(R_o^2-x^2)  
            scoreMatrix=[]
            fileName=[]
            ...
            if((R-pow(x,2)-pow(y,2))<0): #because cancellation problems
                continue
            z=int(math.sqrt(R-pow(x,2)-pow(y,2)))
            ...
            print(str(a)+" Finished points x>"+str(x)+" y>"+str(y)+" with "+str(scoreMatrix))
    ...
    for idx1,fbcaCur in enumerate(endFBCAs):
        ...
        print(str(a)+" Finished comparison on "+str(idx1)+" out of " +str(len(endFBCAs)-1)+str(" total"))
